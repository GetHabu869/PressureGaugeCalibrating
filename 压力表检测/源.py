...
class Manometer_1(object):
    ...
    def __init__(self):
        ...
        for Path in img_file:
            ...
            try:
        # 第一步：读取图片
        #         self.original=cv2.imread(ImPh,cv2.IMREAD_REDUCED_COLOR_2)
    ...
    def location_hfy(self,src):
        ...
        circles2 = cv2.HoughCircles(img, cv2.HOUGH_GRADIENT, 1, mD, param2=15, minRadius=int(r0_2/8)*5, maxRadius=int(r0_2/2)*5)
...
def save(path): #安全保险
    ...
    "read_scale": {
        "infinite_limit": 0.0001

    }
}
'''
    with open(path, 'w',encoding="utf-8") as ru_file:
...
if __name__ == "__main__":
    ...
    with open(filePath, encoding="utf-8") as ru_file:
        ...
        with open(path + filename + nowdate + filesuffix, 'w', encoding="utf-8") as savefile:
            s = json.load(ru_file)
            js = json.dumps(s, indent=5)
            savefile.write(js)
